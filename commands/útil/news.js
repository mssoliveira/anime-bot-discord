  ...
  run: (client, message, args) => {
    ...
    if (!notifyRole) {
      const notifyEmbed1 = new Discord.MessageEmbed()
        .setColor('#29C9FC')
        .setAuthor(
          'Não foi encontrado o id do cargo ou ele não existe no servidor!'
        )
    ...
    if (!message.member.roles.cache.has(process.env.NOVIDADES)) {
      ...
      const notifyEmbed2 = new Discord.MessageEmbed()
        ...
        .setDescription(
          '*Agora você receberá notificações quando houver notícias da comunidade!*'
        )
        .setFooter(process.env.COPY_TEXT, process.env.COPY_IMAGE)
        .setTimestamp();
      ...
      const notifyEmbed3 = new Discord.MessageEmbed()
        ...
        .setDescription(
          '*Você removeu o cargo, e não irá receber mais notificações da comunidade.*'
        )
        .setFooter(process.env.COPY_TEXT, process.env.COPY_IMAGE)
        .setTimestamp();
