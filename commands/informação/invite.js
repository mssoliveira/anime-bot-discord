  ...
  run: async function (client, message, args) {
    ...
    if (!invites) {
      return message.channel.send(
        `> ${message.author}, esse servidor não possui convites!`
      );
    }
    ...
    const embed = new Discord.MessageEmbed().setAuthor(
      `✉️ Convites do servidor ${message.guild.name}`
    );
    rank.map((user, index) =>
      embed.addField(
        `**${index + 1}º** ${user.inviter.username}`,
        `\`\`\`Convidados: ${user.uses}\`\`\` **Link do convite**: ${user.url}`,
        false
      )
    );
    embed
